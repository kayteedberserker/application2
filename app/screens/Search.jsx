...
const SearchScreen = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    authors.forEach((author, index) => {
      authorsWithAds.push(author);
      // Inject after 3, 6, 9...
      if ((index + 1) % 3 === 0) {
        authorsWithAds.push({ _id: `ad-auth-${author._id || index}`, isAuthorAd: true });
      }
    });
    ...
    posts.forEach((post, index) => {
      ...
      // Inject after 3, 6, 9...
      if ((index + 1) % 3 === 0) {
        postsWithAds.push({ _id: `ad-post-${post._id || index}`, isPostAd: true });
      }
    });

    if (activeTab === 'all') return [...authorsWithAds, ...postsWithAds];
    if (activeTab === 'authors') return authorsWithAds;
    return postsWithAds;
  }, [results, activeTab]);

  const renderItem = ({ item }) => {
    if (item.isAuthorAd) return <NativeAdAuthorStyle isDark={isDark} />;
    if (item.isPostAd) return <NativeAdPostStyle isDark={isDark} />;
